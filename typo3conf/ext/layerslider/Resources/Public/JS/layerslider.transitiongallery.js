...
var transitionGallery = {
	...
	appendTransition : function(title, transitions, classes) {
		...
		for(c = 0; c < transitions.length; c+=2) {
			...
			tds.eq(0).append( jQuery('<a>', { 'href' : '#', 'onclick' : 'setTransition(this);', 'rel' : 'tr'+(c+1)+'', 'html' : '<i class="fa fa-check-square-o"></i> '+(c+1)+'. '+transitions[c]['name'] }))

			// Set second transition
			if(typeof transitions[(c+1)] != "undefined") {
				tds.eq(1).append( jQuery('<a>', { 'href' : '#', 'onclick' : 'setTransition(this);', 'rel' : 'tr'+(c+2)+'', 'html' : '<i class="fa fa-check-square-o"></i> '+(c+2)+'. '+ transitions[(c+1)]['name'] }))
			}
		}
	},
